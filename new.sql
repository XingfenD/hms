...
CREATE TABLE IF NOT EXISTS `yiliao2`.`appointment` (
  ...
  `ap_status` INT NOT NULL DEFAULT 0 COMMENT '0为已预约 1为正在进行 2为已结束,3为过号，4为患者已签到',
  PRIMARY KEY (`ap_id`),
...
CREATE TABLE IF NOT EXISTS `yiliao2`.`exam_record` (
  ...
  `exam_id` INT UNSIGNED NOT NULL COMMENT '由触发器自动生成',
...
CREATE TABLE IF NOT EXISTS `yiliao2`.`drug_def` (
  ...
  `drug_specification` VARCHAR(45) NULL COMMENT '规格',
  `drug_manufacturer` VARCHAR(45) NULL COMMENT '制造商',
  PRIMARY KEY (`drug_id`),
...
CREATE TABLE IF NOT EXISTS `yiliao2`.`prescription_record` (
  ...
  `pres_id` INT UNSIGNED NOT NULL COMMENT '通过触发器自动生成',
  `ap_id` INT UNSIGNED NOT NULL,
...
CREATE TABLE IF NOT EXISTS `yiliao2`.`exam_result` (
  `exam_rcd_id` INT UNSIGNED NOT NULL,
...
-- -----------------------------------------------------
-- Table `yiliao2`.`ap_info`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `yiliao2`.`ap_info` ;

CREATE TABLE IF NOT EXISTS `yiliao2`.`ap_info` (
  `ap_id` INT UNSIGNED NOT NULL,
  `pat_requirements` TEXT(500) NULL COMMENT '主诉',
  `pat_history` TEXT(500) NULL COMMENT '过往病史',
  `pat_now_history` TEXT(500) NULL COMMENT '现病史',
  `diagnosis` TEXT(500) NULL COMMENT '初步诊断',
  `treat_method` TEXT(500) NULL,
  PRIMARY KEY (`ap_id`),
  UNIQUE INDEX `ap_id_UNIQUE` (`ap_id` ASC) VISIBLE,
  CONSTRAINT `fk_apinfo.apid_ref_ap`
    FOREIGN KEY (`ap_id`)
    REFERENCES `yiliao2`.`appointment` (`ap_id`)
    ON DELETE CASCADE
    ON UPDATE NO ACTION)
...
-- -----------------------------------------------------
-- Table `yiliao2`.`pres_result`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `yiliao2`.`pres_result` ;

CREATE TABLE IF NOT EXISTS `yiliao2`.`pres_result` (
  `pres_rcd_id` INT UNSIGNED NOT NULL,
  `pres_id` INT UNSIGNED NOT NULL,
  `use_method` VARCHAR(45) NULL COMMENT '用法用量',
  `doc_comment` VARCHAR(45) NULL,
  PRIMARY KEY (`pres_rcd_id`, `pres_id`),
  UNIQUE INDEX `pres_rcd_id_UNIQUE` (`pres_rcd_id` ASC) VISIBLE,
  UNIQUE INDEX `pres_id_UNIQUE` (`pres_id` ASC) VISIBLE,
  CONSTRAINT `fk_presres.rcd_id_ref_presrcd`
    FOREIGN KEY (`pres_rcd_id`)
    REFERENCES `yiliao2`.`prescription_record` (`pres_rcd_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
-- -----------------------------------------------------
-- Placeholder table for view `yiliao2`.`exam_info`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `yiliao2`.`exam_info` (`ap_id` INT, `exam_def_id` INT, `exam_name` INT, `exam_price` INT, `exam_id` INT, `exam_result` INT, `exam_finish_time` INT);

-- -----------------------------------------------------
-- Placeholder table for view `yiliao2`.`pres_info`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `yiliao2`.`pres_info` (`id` INT);
...
-- -----------------------------------------------------
-- View `yiliao2`.`exam_info`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `yiliao2`.`exam_info`;
DROP VIEW IF EXISTS `yiliao2`.`exam_info` ;
USE `yiliao2`;
CREATE  OR REPLACE VIEW `exam_info` AS
SELECT 
	ercd.ap_id,
	edef.exam_def_id,
	edef.exam_name,
    ...
    ercd.exam_id,
    eres.exam_result,
    ercd.oper_time AS exam_finish_time
    
    
FROM exam_record AS ercd

LEFT JOIN exam_result AS eres
ON eres.exam_rcd_id = ercd.exam_rcd_id
LEFT JOIN examination_def AS edef
ON edef.exam_def_id = ercd.exam_def_id

WHERE ercd.status_code = 2;

-- -----------------------------------------------------
-- View `yiliao2`.`pres_info`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `yiliao2`.`pres_info`;
DROP VIEW IF EXISTS `yiliao2`.`pres_info` ;
USE `yiliao2`;
CREATE  OR REPLACE VIEW `pres_info` AS
WITH drug_amount AS(
	SELECT 
		ap_id, drug_id, pres_id, oper_amount 
...
SELECT
    ...
    drug_def.drug_name,
    drug_def.drug_price,
    drug_def.drug_specification,
    drug_def.drug_manufacturer,
    drug_amount.oper_amount,
    res.use_method,
    res.doc_comment

FROM prescription_record AS rcd
LEFT JOIN drug_def
ON drug_def.drug_id = rcd.drug_id
LEFT JOIN drug_amount
ON drug_amount.pres_id = rcd.pres_id
LEFT JOIN pres_result AS res
ON res.pres_rcd_id = rcd.pres_rcd_id
...
USE `yiliao2`$$
DROP TRIGGER IF EXISTS `yiliao2`.`exam_result_BEFORE_INSERT` $$
USE `yiliao2`$$
CREATE DEFINER = CURRENT_USER TRIGGER `yiliao2`.`exam_result_BEFORE_INSERT` BEFORE INSERT ON `exam_result` FOR EACH ROW
BEGIN
...
USE `yiliao2`$$
DROP TRIGGER IF EXISTS `yiliao2`.`pres_result_BEFORE_INSERT` $$
USE `yiliao2`$$
CREATE DEFINER = CURRENT_USER TRIGGER `yiliao2`.`pres_result_BEFORE_INSERT` BEFORE INSERT ON `pres_result` FOR EACH ROW
BEGIN
	DECLARE new_rcd_id INT;
...
START TRANSACTION;
USE `yiliao2`;
INSERT INTO `yiliao2`.`appointment` (`ap_id`, `doc_id`, `pat_id`, `ap_sc_id`, `ap_date`, `ap_time`, `ap_status`) VALUES (1, 1, 3, 1, '2025-03-01', '13:43:23', 0);
INSERT INTO `yiliao2`.`appointment` (`ap_id`, `doc_id`, `pat_id`, `ap_sc_id`, `ap_date`, `ap_time`, `ap_status`) VALUES (2, 1, 4, 1, '2025-03-03', '09:45:21', 0);
...
INSERT INTO `yiliao2`.`examination_def` (`exam_def_id`, `exam_name`, `exam_price`) VALUES (3, '体格检查', 20.00);
...
START TRANSACTION;
USE `yiliao2`;
INSERT INTO `yiliao2`.`drug_def` (`drug_id`, `drug_name`, `drug_price`, `drug_store`, `drug_specification`, `drug_manufacturer`) VALUES (1, '头孢', 9.98, 0, NULL, NULL);
INSERT INTO `yiliao2`.`drug_def` (`drug_id`, `drug_name`, `drug_price`, `drug_store`, `drug_specification`, `drug_manufacturer`) VALUES (2, '布洛芬颗粒', 10.85, 0, NULL, NULL);
INSERT INTO `yiliao2`.`drug_def` (`drug_id`, `drug_name`, `drug_price`, `drug_store`, `drug_specification`, `drug_manufacturer`) VALUES (3, '莲花清瘟胶囊', 10.98, 0, NULL, NULL);
INSERT INTO `yiliao2`.`drug_def` (`drug_id`, `drug_name`, `drug_price`, `drug_store`, `drug_specification`, `drug_manufacturer`) VALUES (4, '左氧氟沙星片', 20.84, 0, NULL, NULL);
INSERT INTO `yiliao2`.`drug_def` (`drug_id`, `drug_name`, `drug_price`, `drug_store`, `drug_specification`, `drug_manufacturer`) VALUES (5, '阿昔洛韦滴眼液', 15.41, 0, NULL, NULL);
INSERT INTO `yiliao2`.`drug_def` (`drug_id`, `drug_name`, `drug_price`, `drug_store`, `drug_specification`, `drug_manufacturer`) VALUES (6, '蒙脱石散', 12.00, 0, NULL, NULL);
INSERT INTO `yiliao2`.`drug_def` (`drug_id`, `drug_name`, `drug_price`, `drug_store`, `drug_specification`, `drug_manufacturer`) VALUES (7, '麝香壮骨膏', 9.00, 0, NULL, NULL);
INSERT INTO `yiliao2`.`drug_def` (`drug_id`, `drug_name`, `drug_price`, `drug_store`, `drug_specification`, `drug_manufacturer`) VALUES (8, '牛黄醒消丸', 42.85, 0, NULL, NULL);
